...
<form class="pkp_form" id="journalSettingsForm" method="post" action="{url router=$smarty.const.ROUTE_COMPONENT component="grid.admin.journal.JournalGridHandler" op="updateContext"}">
	...
	{fbvFormArea id="journalSettings"}
		...
		{fbvFormSection for="enabled" list=true}
			...
			{fbvElement type="checkbox" id="enabled" checked=$enabled value="1" label="Field1"}
		{/fbvFormSection}
        {** FECYT **}
                {fbvFormSection list="true"}
                        {fbvElement type="checkbox" id="FIELD2" label="FIELD2"  checked=$fecyt|compare:true}
                {/fbvFormSection}
         
        {** JCR **}
                {fbvFormSection list="true"}
                        {fbvElement type="checkbox" id="FIELD3" label="FIELD3"  checked=$jcr|compare:true}
                {/fbvFormSection}
